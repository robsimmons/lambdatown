...
module LAX (sig : String → Maybe (Type ⁻)) where
   ...
   infixr 21 _·_[_]
   mutual
      -- V : Value Γ J is the right-focused derivation Γ ⊢ V : A true
      data Value (Γ : Ctx) : Type ⁺ → Set where
         ⁺ : ∀{Q} (x : (con Q) ∈ Γ) → Value Γ (con Q)
         ⁻ : ∀{A} (N : Term Γ A) → Value Γ (↓ A)
   ...
   mutual
      ...
      sbN Γ' W (N₁ , N₂) = sbN Γ' W N₁ , sbN Γ' W N₂
      ...
      sbσ : ∀{J Γ Δ} (Γ' : Ctx)
         ...
         → Subst (Γ' ++ Γ) Δ
      sbσ Γ' W ⟨⟩ = ⟨⟩
      sbσ Γ' W (V , σ) = sbV Γ' W V , sbσ Γ' W σ
